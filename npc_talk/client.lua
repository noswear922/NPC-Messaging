...
function StartMessageDialog(webhook, roleId)
    local input = exports.ox_lib:inputDialog("Оставете съобщение", {
        {label = "Оставете вашето съобщение тук", type = "textarea"}
    })
    ...
    if input then
        ...
        QBCore.Functions.Notify("Съобщението беше изпратено успешно!")
        

        QBCore.Functions.Progressbar("progress_bar", "Обработка на информация...", 5000, false, true, {
            ...
            QBCore.Functions.Notify("Операцията завърши успешно!")
        end, function() 
            QBCore.Functions.Notify("Операцията бе прекъсната!")
        end)
    end
end
